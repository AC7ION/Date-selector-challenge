  ...
  angular.module('ui.bootstrap.datetimepicker', [])
    ...
    .directive('datetimepicker', ['$log', 'dateTimePickerConfig', function datetimepickerDirective($log, defaultConfig) {
      ...
      function DateObject() {
        ...
        this.utcDateValue = tempDate.getDate();
      ...
      var validateConfiguration = function validateConfiguration(configuration) {
        var validOptions = ['startView', 'minView','dropdownSelector'];
      ...
      return {
        ...
        '   <thead>' +
        '       <tr>' +
        '           <th class="left" data-ng-click="changeView(data.currentView, data.leftDate, $event)" data-ng-show="data.leftDate.selectable"><img src="src/img/arrow-left.png"></th>' +
        '           <th class="switch" colspan="5" data-ng-show="data.previousViewDate.selectable" data-ng-click="changeView(data.previousView, data.previousViewDate, $event)">{{ data.previousViewDate.display }}</th>' +
        '           <th class="right" data-ng-click="changeView(data.currentView, data.rightDate, $event)" data-ng-show="data.rightDate.selectable"><img src="src/img/arrow-right.png"></th>' +
        '       </tr>'  +
        '   </thead>' +
        '   <tbody>' +
        '       <tr>' +
        '           <td class="dow" data-ng-repeat="day in data.dayNames" >{{ day }}</td>' +
        '       </tr>' +
        '       <tr data-ng-if="data.currentView !== \'day\'" >' +
        '           <td colspan="7" class="no-highlight">' +
        ...
        '       </tr>' +
        '       <tr>' +
        '           <td colspan="7" class="today-btn no-highlight ng-binding" data-ng-click="setToday()"><div>Today</div></td>' +
        '       </tr>' +
        '   </tbody>' +
        '</table></div>',
        scope: {
          ...
          var dataFactory = {
            ...
            day: function day(unixDate) {
              ...
              var result = {
                ...
                'previousViewDate': new DateObject({
                  utcDateValue: previousViewDate.valueOf(),
                  display: startOfMonth.format('MMM YYYY')
                }),
                ...
                'weeks': [],
                'today': {
                    'utcDateValue': moment().valueOf(),
                    'display': moment().format('D'),
                    'active': moment().format('YYYY-MMM-DD') === activeDate,
                    'past': moment().isBefore(startOfMonth),
                    'future': moment().isAfter(endOfMonth)
                  }
              };
